...
from boundary.models import (ElectionBoundary, Boundary, BoundaryType)
from .utils import *
...
def get_school_report(school_id, survey_id, from_yearmonth, to_yearmonth):
    ...
    try:
        ...
        for index,date in enumerate(yearmonth_dates):
            ...
            school_data["district_name"] = school_info.district_name.capitalize()
            school_data["district_lang_name"] = school_info.district_lang_name
            school_data["block_name"] = school_info.block_name.capitalize()
            school_data["block_lang_name"] = school_info.block_lang_name
            school_data["cluster_name"] = school_info.cluster_name
            school_data["gp_id"] = school_info.gp_id.id
            school_data["gp_name"] = school_info.gp_name.capitalize()
            school_data["gp_lang_name"] = school_info.gp_lang_name
...
def get_all_districts(district_id):
    try:
        schoolids = []
        for districtid in district_id:
            print(districtid)
            districtinfo = Boundary.objects.get(id=413)
            print("districtinfo", districtinfo,districtinfo.name)
            school_id = Institution.objects.filter(admin1_id=districtid)
            for id in school_id:
                schoolids.append(id.id)
            return schoolids

    except:
        print("unable to fetch school info for district ID %s" % district_id)
